...
from Logic.Statistics.Costs.Cost import ClashCost, Cost
...
class Timetable:
    ...
    def GetAllSessionByGroup(self, group_identifier: int):
        ...
        for session in self.sessions:
            if session.group.identifier == group_identifier: sessions.append(session)
        return sessions
    
    def GetAllSessionByRoom(self, room_identifier: int):
        sessions = []
        for session in self.sessions:
            if session.room.identifier == room_identifier: sessions.append(session)
        return sessions
    
    def GetAllSessionByInstructor(self, instructor_identifier: int):
        sessions = []
        for session in self.sessions:
            if session.instructor.identifier == instructor_identifier: sessions.append(session)
        return sessions
    ...
    def SetPreferenceSatisfactionCost(self, identifier: int, p):
        for index, session in enumerate(self.sessions):
            ...
            if session.identifier == identifier: self.sessions[index] = p
    
    def GetIndexOfSession(self, session_identifier: int):
        for session in self.sessions:
            if session.identifier == session_identifier: return self.sessions.index(session)
    ...
    def Output(self):
        clashes = {type_: [{"identifier": clash.object_.identifier, "sessions": clash.sessions_holder} for clash in clashes] for type_, clashes in self.clashes.items()}
    ...
    def Statistics(self):
        ...
        room_cost =  None
        for session in self.sessions:
            if clash_cost is None: clash_cost = session.clash_cost 
            elif clash_cost is not None: clash_cost+= session.clash_cost

            if preference_cost is None: preference_cost = session.preference_satisfcation_cost
            elif preference_cost is not None: preference_cost += session.preference_satisfcation_cost
    ...
    def Print(self):
        ...
        for time in self.times:
            ...
            for day in self.days:
                ...
                if sessions_info != "": row_data.append(sessions_info)
